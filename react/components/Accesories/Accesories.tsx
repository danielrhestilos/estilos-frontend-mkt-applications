...
function Accesories() {
        ...
        fetch(`/api/catalog_system/pub/products/crossselling/accessories/${productId}`)
            ...
            .then(data => {
                if (data.length !== 0) {
                    ...
                    let origPrice = nuevo.reduce((total, item) => total + item.listPrice, 0);
    ...
    const onClickAddToCart = () => {
        const arreglo: any = [];
        for (const prod of productCJ)
            if (ids.includes(prod.id)) arreglo.push(prod);
                                                    ...
                                                    <a href={"/" + item.linkText + "/p"}>
                                                        ...
                                                        {(item.price == item.listPrice)
                                                            ...
                                                            : <div className={styles.productContainerPrice}>
                                                                <span className={styles.productOldPrice}>{item.listPrice.toFixed(2)}</span>
                                                                <span className={styles.productPrice}>{item.price.toFixed(2)}</span>
                                                            </div>
                                                        }
                            ...
                            <div className={styles.containerPrice}>
                                ...
                                {total != null && (
                                    <div className={styles.contentPrincePercentMobile}>
                                        <span className={styles.titlePrice}>Precio total</span>
                                        <div className={styles.valuesPrices}>
                                            <span className={styles.valuePrice}>{total?.toFixed(2)}</span>
                                            {((originalPrice - total) != 0) && (<span className={styles.discountPriceMobile}>Ahorra S/ {(originalPrice - total)?.toFixed(2)}</span>)}
                                        </div>
                                    </div>
                                )}
