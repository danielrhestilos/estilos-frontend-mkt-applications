...
const CarouselProducts: React.FC<CarouselProductsProps> = ({
  ...
  useEffect(() => {
    if (id === 'last-visited-products') {
      ...
      fetch(
        `${urlBestSellerCategory}&_from=0&_to=${
          maxItems - 1
        }&fq=C:/*/${idCategory}/`
      )
        .then((response) => response.json())
        ...
        <>
          <h2
            className={`${styles.title} ${
              addToCartButton ? styles.smallTitle : ''
            }`}
          >
            {title}
          </h2>
              ...
              {listItems.map((item: any) => (
                <div
                  className={`${styles.product} ${
                    addToCartButton ? styles.minicartProduct : ''
                  }`}
                >
                  ...
                  {addToCartButton && (
                    <button
                      id={`${item?.productId}`}
                      className={`${styles.addToCartButton} cart-addProductTk`}
                      onClick={(e: any) => handleAddToCart(e, item)}
                    >
                      agregar al carrito
                    </button>
                  )}
