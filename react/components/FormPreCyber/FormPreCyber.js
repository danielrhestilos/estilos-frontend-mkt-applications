...
const REG_USER_EMAIL = /^[a-z0-9]+(\.[_a-z0-9]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,15})$/;
const REG_PHONE = /^[9][0-9]{8}$/;
const REG_DNI = /^[1-9][0-9]{7}$/;
var cont = 0;
var disableForm = false; 
export default function FormPreCyber(){
        ...
        const handleChecked = e => {
          setForm({
            ...form,
            catTodos: 'No',
            [e.target.name]: checkedLogic(e.target) ? 'Si': 'No'
          });
        };
        ...
        const handleTerms = e => {
          setForm({
            ...form,
            terminosYPoliticas: e.target.checked ? 'Si': 'No'
          });
        }
        ...
        const handleDatas = e => {
          setForm({
            ...form,
            autorizarDatos: e.target.checked ? 'Si': 'No'
          });
        }
        ...
        const checkedLogic = e => {
          (e.name!='catTodos' && e.name!='terminosYPoliticas' && e.name!='autorizarDatos') ? document.getElementById('catTodos').checked = false : null;
          return e.checked;
        }
        ...
        const validateName = (name) => {
          let resp = (REG_USER_NAME.test(name) && name.trim() != '') ? true : false;
          if(!resp){
                showError('errorName', 'name');
          }
          return resp; 
        } 

        const validateLastName = (document) => {
          let resp = (REG_DNI.test(document) && document.trim() != '') ? true : false;
          if(!resp){
                showError('errorLastName', 'document');
          }
          return resp; 
        } 

        const validateEmail = (email) => {
          let resp = (REG_USER_EMAIL.test(email) && email.trim() != '') ? true :false;
          if(!resp){
        ...
        const validateCategory = data =>{
          var contCategories = [
            ...
            data.catTecnologia,
            data.catDormitorio,
            data.catElectrohogar,
            data.catHogar,
            data.catDeporte, 
            data.catModa,         

          ]
          const validAmount = l => l == 'Si';
          var rpt = contCategories.some(validAmount);
          if(!rpt)
        ...
        const setCheckbox = c => {
          ...
          if(c == 'send'){
            flag = false;
            document.getElementById('TerminosYPoliticas').checked = false;
            document.getElementById('autorizarDatos').checked = false;
            document.getElementById('catTodos').checked = false;
          }else if('everyone'){
            flag = true;
          }
 
          document.getElementById('catTecnologia').checked = flag;
          document.getElementById('catDormitorio').checked = flag;
          document.getElementById('catHogar').checked = flag;
          document.getElementById('catElectrohogar').checked = flag;
          document.getElementById('catDeporte').checked = flag;
          document.getElementById('catModa').checked = flag;
        ...
        const handleSubmit = async (e) => {
          ...
          const respEmail = validateEmail(data.email) ? hideError('errorEmail', 'email') : null;
          const respPhone = validatePhone(data.phone) ? hideError('errorPhone', 'phone') : null;
          const respTerm = validateTerms(data.terminosYPoliticas) ? hideError('errorTerms') : null;
          const respValidateCategory = validateCategory(data) ? hideError('errorCategory') : null;
        
          if (respName === null || respLastName === null || respEmail === null || respPhone === null || respTerm === null || respValidateCategory === null) {
            console.log(
              'XX',
              {
                respName,
            ...
            try {
              const response = await fetch(
                ...
                method: 'PUT',
                headers: {
                  // 'Authorization': 'Klaviyo-API-Key pk_bfcaf4685b1a1b77195e1772f2abf4eb88',
                ...
                body: JSON.stringify({
                  data: {
                    correo:form.email,
                    nombre_apellidos:form.name,
                    preferencias:arrayCategories(form),
                    telefono:form.phone,
                    documento: form.DNI
                    // type: 'profile',
                    // attributes: {
                    ...
                    //   properties: {
                    //     DNI: form.DNI,
                    //     nombre_apellido: form.name,
                    //     $phone: form.phone,
                    //     $source: "Embed FlyOut CyberMania",
                    //     categoria_cyber: arrayCategories(form),
                    //     Aceptar_politicas: "Si"
                    //   }
                    // }
                  }
                })
        ...
        const clickClose = () => {
          cont = 0; // ocultando confetti
          setShowConfetti({ready:'no'});
          var p = document.getElementById('poupP'); // Ocultando poup
          p.style.zIndex = '-2'; 
          p.style.opacity = '0';
        }
        const arrayCategories = (data) => {
          let catPreferidas = [] 
          if(data.catTecnologia == 'Si') catPreferidas.push('Tecnologia')
          if(data.catDormitorio == 'Si') catPreferidas.push('Dormitorio')
          if(data.catElectrohogar == 'Si') catPreferidas.push('Electrohogar')
          if(data.catHogar == 'Si') catPreferidas.push('Hogar')
          if(data.catDeporte == 'Si') catPreferidas.push('Deporte')
          if(data.catModa == 'Si') catPreferidas.push('Moda')
         
          return JSON.stringify(catPreferidas)
        }
        return (
          <>      
                  <div className={styles.containerForm}>
                    <div id='poupP' className={styles.poup}>  
                    <p className={styles.thanks}>Gracias por suscribirte a </p> 
                    ...
                    <p className={styles.earlyDesktop}>¡Pronto recibirás las mejores ofertas y</p>
                    <p className={styles.earlyMobile}>¡Pronto recibirás las</p>
                    <p className={styles.earlyMobile}>mejores ofertas y</p>
                    <p className={styles.promotion}>promociones!</p>
                    </div>   
{/* 
          ...
          if(data.catDeporte == 'Si') catPreferidas.push('Deporte')
          if(data.catModa == 'Si') catPreferidas.push('Moda')     */}
                    <form class={styles.formPreCyber} action="" id="form" onSubmit={handleSubmit}>
                        <div className={styles.wrapEmailPhone}>
                          <div className={[styles.inputBx, styles.nameBx].join(' ')} id="nameBx">
                              <label className={styles.labelInputText} htmlFor="name">NOMBRES</label>
                              <input type="text" id="name" disabled={disableForm} name='name' value={form.name} onChange={handleChange} placeholder="Ingresar nombres"/>
                              <span id='errorName' className={styles.errorText}>Ingrese un nombre válido porfavor</span>
                          </div>
                          <div className={[styles.inputBx, styles.lastNameBx].join(' ')} id="lastNameBx">
                              <label className={styles.labelInputText} htmlFor="document">DNI</label>
                              <input type="text" id="document" disabled={disableForm} name='document' value={form.document} onChange={handleChange} placeholder="Ingresar DNI"/>
                              <span id='errorLastName' className={styles.errorText}>Ingrese documento válido porfavor</span>
                          </div>
                        </div>
                        
                        <div className={styles.wrapEmailPhone}>
                          <div className={[styles.inputBx, styles.emailBx].join(' ')} id="emailBx"> 
                              <label className={styles.labelInputText} htmlFor="email">CORREO</label>
                              <input type="text" value={form.email} disabled={disableForm} onChange={handleChange}  name="email" id="email" placeholder="Ingresar correo"/>
                              <span id='errorEmail' className={styles.errorText}>Ingrese un correo válido porfavor</span>
                          </div>
                          <div className={[styles.inputBx, styles.phoneBx].join(' ')} id="phoneBx"> 
                              <label className={styles.labelInputText} htmlFor="phone">CELULAR</label> 
                              <input maxlength="9" type="text" value={form.phone} disabled={disableForm} onChange={handleChange}  name="phone" id="phone" placeholder="Ingresa celular"/>
                              <span id='errorPhone' className={styles.errorText}>Ingrese un número válido</span>
                          </div>
                        </div> 
                        <label>Marca tus categorías preferidas y recibe las ofertas</label>
                        <span id='errorCategory' className= {[styles.errorText, styles.errorCategory].join(' ')}>Elija una o más categorías</span>
                        <div className={styles.containerCheckbox}>
                            <div className={styles.containerOpc}> 
                                <input type="checkbox" onChange={handleChecked} disabled={disableForm} name="catTecnologia" id="catTecnologia" value="Celulares"/>
                                <label htmlFor="catTecnologia">
                                    Tecnologia
                                </label>
                            </div>
                            <div className={styles.containerOpc}> 
                                <input type="checkbox" onChange={handleChecked} disabled={disableForm} name="catDormitorio" id="catDormitorio" value="Audifonos"/>
                                <label htmlFor="catDormitorio">
                                    Dormitorio
                                </label>
                            </div>
                            <div className={styles.containerOpc}> 
                                <input type="checkbox" onChange={handleChecked} disabled={disableForm} name="cateElectrohogar" id="cateElectrohogar" value="Parlantes"/>
                                <label htmlFor="cateElectrohogar">
                                    Electrohogar
                                </label>
                            </div>
                            <div className={styles.containerOpc}> 
                                <input type="checkbox" onChange={handleChecked} disabled={disableForm} name="catHogar" id="catHogar" value="Gamer"/>
                                <label htmlFor="catHogar">
                                    Hogar
                                </label>
                            </div>
                            <div className={styles.containerOpc}> 
                                <input type="checkbox" onChange={handleChecked} disabled={disableForm} name="catDeporte" id="catDeporte" value="Hogar inteligente"/>
                                <label htmlFor="catDeporte">
                                    Deporte
                                </label>
                            </div>
                            <div className={styles.containerOpc}> 
                                <input type="checkbox" onChange={handleChecked} disabled={disableForm} name="catModa" id="catModa" value="Bicis y scooters"/>
                                <label htmlFor="catModa">
                                    Moda
                                </label>
                            </div>

                            <div className={[styles.containerOpc, styles.catHidden].join(' ')}> 
                                <input type="checkbox" onChange={handleEveryone} disabled={disableForm} name="catTodos" id="catTodos" value="Todos"/>
                                <label htmlFor="catTodos">
                                    Todos
                                </label>
                            </div>
                        ...
                        <div className={styles.terminos}> 
                            <div className={styles.terminosVisible}>
                              <input type="checkbox" id="TerminosYPoliticas" onChange={handleTerms} disabled={disableForm} name="terminosYPoliticas" value="Terminos y Politicas"/>
                              <label htmlFor="TerminosYPoliticas">
                                  He leído y acepto los <a href='https://www.coolbox.pe/terminos-y-condiciones'>Términos y condiciones</a> y <a href='https://www.coolbox.pe/politicas-de-privacidad'>Política de Privacidad</a>
                              </label>
                            </div>
                            <p id='errorTerms' className={styles.errorCheckbox}>Es necesario seleccionar este campo porfavor</p>  
                        </div>
