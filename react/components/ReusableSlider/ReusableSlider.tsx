...
function ResusableSlider(props: any): any {
  ...
  if (visible && props.data.length >= 3)
    return (
      <div className={`${styles.containerPurchased} `}>
        <div className={styles.titlePurchased}>
          <div className={styles.headerPurchased}>
            <strong>{props.title}</strong>
            <div className={styles.closePurchased} onClick={toggleVisibility}>
              <svg
                fill="none"
                width="32"
                height="32"
                viewBox="0 0 16 16"
                className=" vtex-modal-layout-0-x-closeIcon vtex-modal-layout-0-x-closeIcon--product__modal--size vtex-modal-layout-0-x-closeIcon--product__modal--stores"
            ...
            >
              {props.data.map((product: any) => (
                <Slide key={product.productId} id={product.productId}>
                  ...
                  >
                    <img
                      className={styles.containerImageCardPurchased}
                      src={product.items[0].images[0].imageUrl}
                    />
                    <div>
                      <strong className={styles.brandPurchased}>
                        {product.brand}
                      </strong>
...
export default ResusableSlider
