...
const CustomFilterColection = (props: { images: any[];small:boolean; links:string[]} | undefined) => {
  ...
  return (
     show&& <div className={`w-100 pb4 flex justify-between overflow-x-auto ${handles.rowContent}`}>
        {props.images.map((index: any, key: any) => {
          return (
            <div className={`w-100 ${handles.content}`} key={key} data-js={`id_${index.url}`}>
              <RenderItem scrollY={scrollY} url={index.url}   logo={index.logo} nombre={index.nombre} rowcontentStyle={handles.rowContent} small={props?.small}/>
            </div>
          )
        })}
      </div>
