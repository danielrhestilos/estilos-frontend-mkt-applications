...
function CustomDescription() {
  ...
  return (
    <div
      ...
      className={styles.customSpecificationsContainer}
    >
      <section
        id="custom-description-section"
        className={styles.contentSpecifications}
        style={{ maxHeight: `${!showMore ? '100%' : '280px'}` }}
      >
      ...
      <div className="vtex-store-components-3-x-pointerEventsNone  bottom-0 w-100 h-100 flex-column justify-end flex">
        <div
          className={
            showMore ? 'vtex-store-components-3-x-fadeBottom w-100 h-50' : ''
          }
          style={{ transition: 'all 400ms ease-in-out 0s' }}
        ></div>
        {isMobil && sectionHeight > 280 && (
          <div
            className={`${styles.containerShowMore} vtex-store-components-3-x-pointerEventsAuto tc w-100 bg-base`}
          >
            <button
              onClick={() => {
                toggleShowMore()
              }}
              className="vtex-store-components-3-x-showMoreButton c-action-primary t-action pointer ma5 bn outline-0"
            >
