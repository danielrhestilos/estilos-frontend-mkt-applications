...
const CardCTA = () => {
    ...
    const itemPrice = commertialOffer?.Price !== undefined ? commertialOffer.Price : null;
    ...
    return rendered ? (
        <a className={styles.cardContainer} href="https://www.tarjetaestilos.com.pe/solicitartarjeta" target="__blank">
            <img src="https://estilospe.vtexassets.com/arquivos/estiloscard.svg" alt="Estilos Card" style={{ height: "27px" }} />
            <div className={styles.ctaContainer}>
                {price != 0 ? <p className={styles.cta}>Ahorra <strong>S/ {ahorro.toFixed(2)}</strong> con tarjeta Estilos</p> : <p className={styles.cta}>Compra con tarjeta Estilos y accede a nuestros beneficios</p>}
                <p className={styles.cta}>¿Aún no la tienes? <span className={styles.here}>¡Solicítala aquí!</span></p>
            </div>
        </a>
    ) : null;
};
